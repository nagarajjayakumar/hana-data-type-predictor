...
object hana_random_uniform_system extends Logging {


  log
  val TASK: String = "hana_random_uniform_system"
  ...
  def getData(spark: SparkSession,
    ...
    val hana_sampling_query: String = s"select * from '${opts.src_dbo_name}' TABLESAMPLE SYSTEM(${opts.sampling_percentage}) order by rand()"
